...
function Popup(props) {
  ...
  let dihes = allDishes.filter((dishes)=>{
    ...
    return(
     ...
     <p className='dark:text-gray-400 overflow-y-scroll text-sm h-[170px]' >{item.strInstructions}</p>
  ...
  return (
    <div className='bg-black-transparant   fixed left-0 right-0 top-0 bottom-0 z-[999] flex flex-col items-center justify-center' >
     <FontAwesomeIcon onClick={props.HidePopup} className='absolute z-50 right-4 top-1 md:top-4 hover:rotate-45 hover:ease duration-75 hover:cursor-pointer   text-red-600 font-bold text-3xl' icon={faXmark}/>
